= Minakshi Initializr image:https://badges.gitter.im/spring-io/initializr.svg[link="https://gitter.im/spring-io/initializr?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge&utm_content=badge"]
...
Minakshi Initializr provides an extensible API to generate quickstart projects. It also
...
Minakshi Initializr also exposes an endpoint that serves its metadata in a well-known
...
== Modules
Minakshi Initializr has the following modules:
...
Minakshi Initializr can be used as follows:
...
NOTE: Minakshi Initializr is not available on Maven central yet so you will have to
...
== License
Minakshi Initializr is Open Source software released under the
